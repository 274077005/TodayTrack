...
#import "SkyerScanningCodeVC.h"
#import <AVFoundation/AVFoundation.h>
#import "SGScanningQRCodeView.h"

@interface SkyerScanningCodeVC ()<AVCaptureMetadataOutputObjectsDelegate, UINavigationControllerDelegate, UIImagePickerControllerDelegate>
/** 会话对象 */
...
@implementation SkyerScanningCodeVC
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    //一个返回按钮
    _btnBack=[[UIButton alloc] initWithFrame:CGRectMake(15, 30, 50, 35)];
    ...
    //一个相册按钮
    _btnImage=[[UIButton alloc] initWithFrame:CGRectMake(self.view.frame.size.width-60, 30, 50, 35)];
    [_btnImage setTitle:@"相册" forState:UIControlStateNormal];
    _btnImage.titleLabel.font=[UIFont systemFontOfSize:14];
    [_btnImage setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [_btnImage addTarget:self action:@selector(rightBarButtonItenAction) forControlEvents:UIControlEventTouchUpInside];
...
- (void)scanQRCodeFromPhotosInTheAlbum:(UIImage *)image {
    ...
    for (int index = 0; index < [features count]; index ++) {
        ...
        if (_scanResult) {
            _scanResult(scannedResult);
        }
    }
    [self btnBackAction];
}
- (void)getScanResultStr:(scanResult)scanResult{
